...
class Student : public Person {
  ...
  public:
    ...
    Student(string studentID, float currentCGPA) : Person() {
        ...
        if (currentCGPA > 0.0 && currentCGPA <= 4.0)
    ...
    virtual string getRole() const { return "Student"; }
...
class ScholarshipStudent : public Student {
  ...
  public:
    ...
    ScholarshipStudent(float scholarshipPercentage, float minCGPARequirement)
        : Student() {
        if (scholarshipPercentage > 0 && scholarshipPercentage <= 100)
            this->scholarshipPercentage = scholarshipPercentage;
        if (minCGPARequirement > 2.0 && minCGPARequirement <= 4.0)
            this->minCGPARequirement = minCGPARequirement;
    }

    string getRole() const { return "Scholarship Student"; }
...
class Faculty : public Person {
  ...
  public:
    ...
    string getRole() const { return "Faculty Member"; }

    void assignNewCourse() { courseTaught++; }
};
