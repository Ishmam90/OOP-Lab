...
class AutonomusUnit {
  ...
  public:
    ...
    virtual ~AutonomusUnit() { cout << "Autonomus Unit Shutting Down" << endl; }

    string getUnitType() const { return "General Autonomus Unit"; }
...
class VehicleUnit : virtual public AutonomusUnit {
  ...
  public:
    ...
    VehicleUnit(string id, bool status, int maxSpeed, int fuelLevel, int x,
                int y)
        : AutonomusUnit(id, status), maxSpeed(maxSpeed), fuelLevel(fuelLevel),
          currentLocationX(x), currentLocationY(y) {}

    ~VehicleUnit() { cout << "Vehicle going Dark"; }
    ...
    int getMax() const { return maxSpeed; }

    float getLevel() const { return fuelLevel; }

    int get_locationX() const { return currentLocationX; }

    int get_locationY() const { return currentLocationY; }
    ...
    void executeMission() { cout << "Executing Vehicle Unit Mission" << endl; }

    string getUnitType() const { return "Vehicle Unit"; }
};
...
class CameraUnit : virtual public AutonomusUnit {
  ...
  public:
    ...
    CameraUnit(bool nightVisionMode, int storageCapacity,
        ...
        if (recordingQuality == "HD" || recordingQuality == "Full HD" ||
            recordingQuality == "4K")
            this->recordingQuality = recordingQuality;
    }

    ~CameraUnit() { cout << "Camera going dark" << endl; }

    void setNightVision(bool nightVision) { nightVisionMode = nightVision; }
    ...
    void setQuality(string quality) {
        if (quality == "HD" || quality == "Full HD" || quality == "4K")
            recordingQuality = quality;
    }

    bool getNightVision() const { return nightVisionMode; }

    int getCap() const { return storageCapacity; }

    string getQuality() const { return recordingQuality; }
    ...
    void executeMission() { cout << "Executing Camera Unit Mission" << endl; }

    string getUnitType() const { return "Surveillance Unit"; }
};
...
class WeaponArm {
  ...
  public:
    ...
    WeaponArm(int ammoCount, int damage, string weaponType) {
        ...
        if (damage > 0)
            this->damage = damage;
        if (weaponType == "Kinetic" || weaponType == "Energy")
            this->weaponType = weaponType;
    }

    ~WeaponArm() { cout << "Weapon going dark" << endl; }
    ...
    void reload(int amount) { ammoCount += amount; }
...
class MissionObjective {
  public:
    ...
    virtual string getMissionType() const { return "Mission Classification"; }

    int difficultyLevel() const { return 5; }
};
...
class ReconMission : public MissionObjective {
  ...
  public:
    ...
    ReconMission(string areaToScan, int intelPriority) {
        ...
        if (intelPriority > 0)
            this->intelPriority = intelPriority;
    }
    ...
    string getMissionType() const { return "Reconnaissance"; }
};
...
class PatrolBot : public VehicleUnit, CameraUnit {
  ...
  public:
    ...
    PatrolBot(string id, bool status, int maxSpeed, float fuel, int x, int y,
              bool nightVision, int storage, string quality, string modelNumber,
              WeaponArm weapon, int missionCapacity)
        : AutonomusUnit(id, status),
          VehicleUnit(id, status, maxSpeed, fuel, x, y),
          CameraUnit(nightVision, storage, quality), modelNumber(modelNumber),
          WArm(weapon) {
        missionCount = 0;
    ...
    int getCapability() const { return missionCount; }

    virtual void getCapabilityReport() const {
        cout << "Patrol Bot Mission Capability: " << missionCount << endl;
    }

    void reloadWeapon(int amount) { WArm.reload(amount); }

    void executeMission() override { VehicleUnit::executeMission(); }
};
...
int main() {
    ...
    PatrolBot bot("P-01", true, 80, 50.0, 0, 0, true, 100, "4K", "X9", weapon,
                  5);
